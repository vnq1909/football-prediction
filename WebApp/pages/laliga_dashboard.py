...
st.set_page_config(page_title="La Liga Dashboard", layout="wide", initial_sidebar_state="expanded", page_icon="⚽")
...
def load_laliga_data():
    data_path = os.path.join('Datasets', 'Cleaned Datasets', 'La_Liga_Stats.csv')
...
def load_laliga_prediction():
    pred_path = os.path.join('Datasets', 'Predictions', 'laliga_predictions.csv')
    if os.path.exists(pred_path):
        return pd.read_csv(pred_path)
    return None

def laliga_overview(df):
...
df = load_laliga_data()
pred_df = load_laliga_prediction()
tab1, tab2 = st.tabs(["Thống kê", "Dự đoán"])
with tab1:
    if df is not None:
        overview_table, matches_per_year, avg_gf, avg_ga, avg_poss = laliga_overview(df)
